...
float light_lof::collect_lrds(
  ...
  cout << "Collected Neighbor LRD from SFV_T Query (neighbors = " << neighbors.size() << ", collected LRDs = " << neighbor_lrds.size() << "):" << endl;
  for (size_t i = 0; i < neighbor_lrds.size(); ++i) {
    cout << "  LRD[" << neighbors[i].first << " (distance = " << neighbors[i].second << ")] => " << neighbor_lrds[i] << endl;
  }
...
float light_lof::collect_lrds(
  ...
  float lrd_for_query = collect_lrds_from_neighbors(neighbors, neighbor_lrds);
  cout << "Collected Neighbor LRD from ID [" << id << "] (neighbors = " << neighbors.size() << ", collected LRDs = " << neighbor_lrds.size() << "):" << endl;
  for (size_t i = 0; i < neighbor_lrds.size(); ++i) {
    cout << "  LRD[" << neighbors[i].first << " (distance = " << neighbors[i].second << ")] => " << neighbor_lrds[i] << endl;
  }
  return lrd_for_query;
...
void light_lof::update_entries(const unordered_set<std::string>& neighbors) {
  ...
  for (std::vector<uint64_t>::const_iterator it = ids.begin();
    ...
    cout << "UPDATE: kDist [" << *it << "] = " << nn_result.back().second << " (ID = " << nn_result.back().first << ")" << endl;
  }
